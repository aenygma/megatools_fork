...
gboolean mega_session_refresh(mega_session* s, GError** err)
{
  ...
  if (ok_node && s_json_get_type(ok_node) == S_JSON_TYPE_ARRAY)
  {
    ...
    for (i = 0, l = g_strv_length(oks); i < l; i++)
    {
      ...
      gc_free gchar* ok_k = s_json_get_member_string(ok, "k");    // b64(aes(share_key_for_h, master_key))
...
mega_node* mega_session_mkdir(mega_session* s, const gchar* path, GError** err)
{
  ...
  else
  {
    ...
    // prepare request
    gc_free gchar* mkdir_node = api_call(s, 'o', NULL, &local_err, "[{a:p, t:%s, i:%s, n: [{h:xxxxxxxx, t:1, k:%s, a:%s}]}]", p->handle, s->rid, dir_key, dir_attrs);
    if (!mkdir_node)
    {
      g_propagate_error(err, local_err);
      return NULL;
...
gboolean mega_session_rm(mega_session* s, const gchar* path, GError** err)
{
  ...
  {
    gc_free gchar* ur_node = api_call(s, 'i', NULL, &local_err, "[{a:ur, u:%s, l:0, i:%s}]", mn->handle, s->rid);
    if (!ur_node)
    {
      g_propagate_error(err, local_err);
      return FALSE;
...
#define tman_debug(args...) G_STMT_START { if (mega_debug && MEGA_DEBUG_TMAN) g_print(args); } G_STMT_END
...
static void tman_worker_thread_fn(gpointer data, gpointer user_data)
{
  ...
  {
    g_mutex_unlock(&t->stream_lock);
    err = g_error_new(MEGA_ERROR, MEGA_ERROR_OTHER, "Failed reading from the stream: %s", local_err->message);
    g_clear_error(&local_err);
    goto err;
  }
  ...
  *((guint64*)&iv[8]) = GUINT64_TO_BE((guint64)(c->offset / 16)); // this is ok, because chunks are 16b aligned
  ...
  if (response->len > 0)
  {
    ...
    {
      err = g_error_new(MEGA_ERROR, MEGA_ERROR_OTHER, "Server returned error code %s", srv_error_to_string(atoi(response->str)));
      goto err;
    }
...
static gchar* create_preview(mega_session* s, const gchar* local_path, const guchar* key, GError** err)
{
  ...
  if (has_ffmpegthumbnailer && g_regex_match_simple("\\.(mpg|mpeg|avi|mkv|flv|rm|mp4|wmv|asf|ram|mov)$", local_path, G_REGEX_CASELESS, 0))
  {
    ...
    if (dir)
    {
      ...
      gc_free gchar* tmp = g_strdup_printf("ffmpegthumbnailer -t 5 -i %s -o %s/thumb.jpg -s 128 -f -a", qpath, dir);
  ...
  else if (has_convert && g_regex_match_simple("\\.(jpe?g|png|gif|bmp|tiff|svg|pnm|eps|ico|pdf)$", local_path, G_REGEX_CASELESS, 0))
  {
    ...
    if (dir)
    {
      ...
      gc_free gchar* tmp = g_strdup_printf("convert %s -strip -background white -flatten -resize 128x128^ -gravity center -crop 128x128+0+0 +repage %s/thumb.jpg", qpath, dir);
...
mega_node* mega_session_put(mega_session* s, mega_node* parent_node, GFile* file, GError** err)
{
  ...
  // prepare request
  gc_free gchar* put_node = api_call(s, 'o', NULL, &local_err,
    "[{a:p, t:%s, n:[{h:%s, t:0, k:%s, a:%s, fa:%s}]}]",
...
gboolean mega_session_register(mega_session* s, const gchar* email, const gchar* password, const gchar* name, mega_reg_state** state, GError** err)
{
  ...
  gc_free gchar* node = api_call(s, 's', NULL, &local_err, "[{a:up, k:%S, ts:%S}]", b64_aes128_encrypt(master_key, 16, password_key), base64urlencode(ts_data, 32));
  if (!node)
  {
    g_propagate_error(err, local_err);
    return FALSE;
